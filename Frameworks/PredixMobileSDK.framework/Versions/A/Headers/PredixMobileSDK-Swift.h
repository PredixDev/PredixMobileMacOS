...
/// If PredixMobilityConfiguration.automaticallyListenForApplicationStateEvents is true, the SDK will automatically handle application lifecycle events, however remote notification event method calls will need to be passed to the SDK from the container app.
/// For macOS local notifications will automatically be handled, unless the container application overrides the NSUserNotificationCenter.default.delegate. See userNotificationCenter: shouldPresent: below for more information.
SWIFT_CLASS("_TtC15PredixMobileSDK27ApplicationDelegateHandlers")
@interface ApplicationDelegateHandlers : NSObject <NSUserNotificationCenterDelegate>
...
/// }
///
/// \endcode\param application the NSApplication object
///
- (void)applicationWillResignActiveWithApplication:(NSApplication * _Nonnull)application;
/// When PredixMobilityConfiguration.automaticallyListenForApplicationStateEvents is false, this method can be used to inform the SDK of the applicationWillResignActive event from a consuming application.
/// Would be called by the container’s AppDelegate, example:
/// \code
...
/// \endcode\param application the NSApplication object
///
- (void)applicationDidBecomeActiveWithApplication:(NSApplication * _Nonnull)application;
...
/// \endcode\param application the NSApplication object
///
- (void)applicationWillTerminateWithApplication:(NSApplication * _Nonnull)application;
/// In macOS containers, Allows the SDK to handle local macOS notifications, informing the running webapp when they occur.
/// This delegate is automatically registered with NSUserNotificationCenter, so it does not need to be called directly, unless the consuming macOS application has set NSUserNotificationCenter.default.delegate itself.
/// This method is called when the Notification Center has decided not to present your notification, for example when your application is front most.
/// The Predix Mobile SDK will return false for this method.
/// \param center the NSUserNotificationCenter object
///
/// \param notification the NSUserNotification object
///
///
/// returns:
/// If this method returns true, the notification will always be displayed.
- (BOOL)userNotificationCenter:(NSUserNotificationCenter * _Nonnull)center shouldPresentNotification:(NSUserNotification * _Nonnull)notification;
...
/// \endcode\param application The NSApplication object.
///
/// \param userInfo Dictionary containing information about the remote notification.
///
- (void)applicationWithApplication:(NSApplication * _Nonnull)application didReceiveRemoteNotification:(NSDictionary<NSString *, id> * _Nonnull)userInfo;
...
///
- (void)applicationWithApplication:(NSApplication * _Nonnull)application didRegisterForRemoteNotificationsWithDeviceToken:(NSData * _Nonnull)deviceToken;
...
///
- (void)applicationWithApplication:(NSApplication * _Nonnull)application didFailToRegisterForRemoteNotificationsWithError:(NSError * _Nonnull)error;
@end
...
/// Application Delegates for App lifecycle events and notifications
@property (nonatomic, readonly, strong) ApplicationDelegateHandlers * _Nonnull applicationDelegates;
